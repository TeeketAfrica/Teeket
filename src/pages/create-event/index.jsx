...
const validationSchemas = [
    Yup.object({
        ...
        eventOrganizer: Yup.string().required(
            "Please input an event organizer"
        ),
            ...
            if (Object.keys(errors).length === 0) {
                ...
                if (id) {
                    try {
                        ...
                        console.log(
                            "Unable to update number of tickets:",
                            error.message
                        );
                    }
                }
        ...
        async (formProps) => {
            ...
            if (Object.keys(errors).length === 0) {
                ...
                try {
                    ...
                    if (id) {
                        const res = await teeketApi.patch(
                            `/events/${id}`,
                            eventPayload
                        );
                        eventId = res.data.id;
                    } else {
                        const res = await teeketApi.post(
                            "/events",
                            eventPayload
                        );
                        eventId = res.data.id;
                    ...
                    if (!eventId) {
                        throw new Error(
                            "Event ID is missing from the response."
                        );
                    }
                    console.log(data);
